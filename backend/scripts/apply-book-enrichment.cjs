/**
 * 应用蒋廷黻《中国近代史大纲》史料增强 events.json
 *
 * 使用方法:
 * node backend/scripts/apply-book-enrichment.cjs
 */
const fs = require('fs');
const path = require('path');

const EVENTS_FILE = path.join(__dirname, '../../frontend/public/data/events.json');
const MAPPING_FILE = path.join(__dirname, './book-enrichment-mapping.json');
...
function main() {
  console.log('=== Phase 2: 蒋廷黻《中国近代史大纲》史料增强 ===\n');
  ...
  console.log(`映射文件共 ${entries.length} 条事件增强\n`);
  ...
  for (const [title, data] of entries) {
    if (data.isNew) {
      ...
      // 尝试匹配已有的人物 ID
      if (data.personNames) {
        // 简单处理：人物 ID 暂时为空，后续可通过 persons.json 匹配
      }
      ...
      if (data.merge === 'replace') {
        ...
        for (const key of ['motive', 'process', 'result', 'impact']) {
          if (data.book[key]) {
            ev.detail[key] = data.book[key];
          }
        }
      } else if (data.merge === 'append') {
        // 追加
        for (const key of ['motive', 'process', 'result', 'impact']) {
          if (data.book[key]) {
            const existing = ev.detail[key] || '';
            if (existing && !existing.includes(data.book[key])) {
              ev.detail[key] = existing + '\n\n' + data.book[key];
            } else if (!existing) {
              ev.detail[key] = data.book[key];
            }
          }
  ...
  console.log(`  未找到（现有事件中不存在）: ${notFoundExisting} 条`);
