/**
 * 从 raw markdown 文件恢复事件详情数据到 events.json
 *
 * 问题根因: convert-raw-data-clean.cjs 硬编码了空 detail 对象
 *
 * 使用方法:
 * node backend/scripts/enrich-from-raw.cjs
 */
const fs = require('fs');
...
// 解析 raw markdown 文件中的事件详情
function parseRawMarkdown(filePath) {
  ...
  for (let i = 1; i < sections.length; i++) {
    ...
    // 提取详情字段
    const detail = {
      motive: extractField(block, '**动机原因**'),
      process: extractField(block, '**经过描述**'),
      result: extractField(block, '**结果**'),
      impact: extractField(block, '**影响**'),
    };

    // 提取元数据
    const time = extractSimpleField(block, '**时间**');
    const location = extractSimpleField(block, '**地点**');
    const persons = extractSimpleField(block, '**参与人物**');
    const summary = extractSimpleField(block, '**事件概述**');
    const eventType = extractSimpleField(block, '**类型**') || '事件';

    // 提取子事件 (#### X.X)
    const subEvents = [];
    const subSections = block.split(/####\s+\d+\.\d+\s*/);
    for (let j = 1; j < subSections.length; j++) {
      const subBlock = subSections[j];
      const subTitleMatch = subBlock.match(/^([^\n]+)\n/);
      if (!subTitleMatch) continue;
      const subTitle = subTitleMatch[1].trim();
      subEvents.push({
        title: subTitle,
        startDate: parseDate(extractSimpleField(subBlock, '**时间**')),
        endDate: null,
        isInstant: true,
        eventType: extractSimpleField(subBlock, '**类型**') || '事件',
        location: extractSimpleField(subBlock, '**地点**'),
        summary: extractSimpleField(subBlock, '**事件概述**') || '',
        content: extractSimpleField(subBlock, '**详细内容**') || '',
        detail: {
          motive: extractField(subBlock, '**动机原因**'),
          process: extractField(subBlock, '**经过描述**'),
          result: extractField(subBlock, '**结果**'),
          impact: extractField(subBlock, '**影响**'),
        },
      });
    }

    // 解析日期
    const startDate = parseDate(time);
...
function extractSimpleField(block, fieldName) {
  const escaped = fieldName.replace(/[.*+?${}()|[\]\\]/g, '\\$&');
...
function parseDate(dateStr) {
  ...
  // 处理 "1839年6月3日-25日" 格式
  const rangeMatch = dateStr.match(/(\d{4})年(\d{1,2})月(\d{1,2})日-(\d{1,2})日/);
  if (rangeMatch) {
    return `${rangeMatch[1]}-${rangeMatch[2].padStart(2, '0')}-${rangeMatch[3].padStart(2, '0')}`;
  }
  // 处理 "1839年10月" 格式
...
function main() {
  console.log('=== Phase 1: 从 raw markdown 恢复事件详情 ===\n');

  // 1. 读取 events.json
  console.log('读取 events.json...');
  ...
  // 2. 解析所有 raw markdown 文件
  console.log('解析 raw markdown 文件...');
  const titleToDetail = new Map();
  let totalRawEvents = 0;

  const periodDirs = fs.readdirSync(RAW_DIR).filter(d => !d.endsWith('.md') && fs.statSync(path.join(RAW_DIR, d)).isDirectory());
  for (const dir of periodDirs) {
    const dirPath = path.join(RAW_DIR, dir);
    const files = fs.readdirSync(dirPath).filter(f => f.endsWith('.md'));
    for (const file of files) {
      const filePath = path.join(dirPath, file);
      const parsed = parseRawMarkdown(filePath);
      totalRawEvents += parsed.length;
      for (const ev of parsed) {
        if (ev.title && ev.detail && (ev.detail.process || ev.detail.motive)) {
          titleToDetail.set(ev.title, ev.detail);
        }
      }
    }
  }

  console.log(`raw markdown 共 ${totalRawEvents} 条事件`);
  console.log(`提取到 ${titleToDetail.size} 条有详情的唯一事件名\n`);
  ...
  for (const ev of events) {
    const rawDetail = titleToDetail.get(ev.title);
    if (rawDetail) {
      // 只填充空字段
      if (!ev.detail) ev.detail = { motive: '', process: '', result: '', impact: '' };
      let hasNewContent = false;
      for (const key of ['motive', 'process', 'result', 'impact']) {
        if (!ev.detail[key] || ev.detail[key].length < 5) {
          ev.detail[key] = rawDetail[key] || '';
          if (rawDetail[key]) hasNewContent = true;
        }
      }
      if (hasNewContent) enrichedCount++;
      else skippedCount++;
    } else {
      notFoundCount++;
    }

    // 升级 impactFactor 到 HPIF 格式
    if (!ev.impactFactor || typeof ev.impactFactor === 'number' || (ev.impactFactor.score && !ev.impactFactor.dimensions)) {
      const oldScore = typeof ev.impactFactor === 'number' ? ev.impactFactor : (ev.impactFactor?.score || 84);
      ev.impactFactor = calcImpact(ev);
    }
  }

  console.log('=== 合并结果 ===');
  console.log(`  已填充详情: ${enrichedCount} 条`);
  console.log(`  跳过（已有详情）: ${skippedCount} 条`);
  console.log(`  未找到（不在 raw 中）: ${notFoundCount} 条`);
  console.log(`  impactFactor 已升级为 HPIF 格式`);
  ...
  const withHPIF = events.filter(e => e.impactFactor && e.impactFactor.dimensions).length;
  console.log(`\n=== 验证 ===`);
  console.log(`  有详情的事件: ${withDetail}/${events.length}`);
  console.log(`  HPIF 格式 impactFactor: ${withHPIF}/${events.length}`);
  console.log(`\n完成! 已写回 events.json`);
}
