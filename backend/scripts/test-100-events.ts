...
function generateScores(event: any): Record<string, number> {
  ...
  const baseScores: Record<string, Record<string, number>> = {
    ...
    '事件': { p: 3.0, e: 2.0, m: 2.0, s: 2.5, i: 2.5, ir: 2.0, t: 2.0, l: 2.5, h: 2.5 },
  };
  ...
  // 军事相关行动（进攻/防守/撤退/会战等）
  if (/(进攻|防守|撤退|会战|战役|战斗|出击|阻击|伏击|围困|突破|转移|作战|歼灭|击毙|俘虏)/.test(fullText)) {
    m += 1.5; h += 0.5;
  }
  // 政治人物变动（去世/就任/下台/被俘/被杀）
  if (/(去世|逝世|病逝|就任|就职|下台|辞职|被俘|被杀|身亡|遇难|牺牲|逝世)/.test(fullText)) {
    p += 1.0; h += 0.5;
  }
  // 组织建设（成立/建立/召开/组建/设立）
  if (/(成立|建立|召开|组建|设立|创办|创建|建立|发展|继续|扩展|扩大)/.test(title)) {
    l += 0.5; p += 0.3;
  }
  // 会议/决策
  if (/(会议|决定|决议|通过|发布|颁布|公布|制定|出台|落实|实施)/.test(fullText)) {
    p += 0.8; l += 0.5;
  }
  // 领土/地理变动
  if (/(收回|归还|割让|租借|边界|边境|领土|主权|管辖|治理)/.test(fullText)) {
    t += 1.5; ir += 0.5;
  }
  // 经济/财政
  if (/(财政|经济|银|钱|资|产|业|商|税|捐|债|款|费|利|息|贷款|借款|拨款)/.test(fullText)) {
    e += 1.0;
  }
  // 文化/学术/出版
  if (/(出版|发表|翻译|著作|学术|文化|文艺|文学|电影|戏剧|艺术|音乐|美术)/.test(fullText)) {
    i += 1.5; l += 0.5;
  }
  // 群众/社会运动
  if (/(运动|罢工|罢课|罢市|游行|示威|抗议|抵制|请愿|集会|结社)/.test(fullText)) {
    s += 1.5; p += 1.0; i += 0.5;
  }
...
const rawFile = '/Users/zhangrz/code/github/mch-prc/frontend/public/data/events.json';
const rawData = JSON.parse(fs.readFileSync(rawFile, 'utf8'));
const eventsData = rawData.events || rawData;
...
const sample = shuffle(eventsData).slice(0, 100);

const calculator = new ImpactFactorCalculator();

// ==================== 测试运行 ====================

console.log('=== 100 个随机事件千分制测试 ===\n');
...
const results: { name: string; type: string; finalScore: number; weightedSum: number; scope: number; duration: number }[] = [];
...
for (const event of sample) {
  ...
  console.log(row);
  results.push({ name: event.title, type: typeAbbr, finalScore: breakdown.finalScore, weightedSum: breakdown.weightedSum, scope: breakdown.scopeBonus, duration: breakdown.durationBonus });
}
...
sorted.forEach((r, i) => {
  const tag = r.type;
  console.log(`  ${String(i + 1).padStart(2)}. [${tag}] ${r.name.padEnd(24)}  ${r.finalScore}/1000  (基础${r.weightedSum} 范围+${r.scope} 持续+${r.duration})`);
});
...
for (const seg of segments) {
  ...
  const bar = '█'.repeat(count) + '·'.repeat(Math.max(0, 50 - count));
  console.log(`    ${seg.label.padEnd(18)} ${count.toString().padStart(2)}  ${bar}`);
}
...
for (const [type, sc] of Object.entries(typeGroups).sort((a, b) => {
